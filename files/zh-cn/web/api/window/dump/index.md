...
[Gecko](/zh-CN/docs/Gecko) 在默认情况 **dump()** 下被禁用 - 它不会做任何事情，但也不会引起错误。要查看`dump`输出，您必须通过设置首选项`browser.dom.window.dump.enabled`来启用它。您可以在[about:config](http://kb.mozillazine.org/About:config)或[user.js](http://kb.mozillazine.org/User.js_file) 文件中设置首选项。注意：`about:config`默认情况下不会列出此首选项，您可能需要创建它（右键单击内容区域 - >新建 - >布尔值）。
...
> **备注：** 如果您希望控制台消息出现在用于启动应用程序的控制台中，则可以使用[Gecko 控制台重定向器](https://github.com/matthewkastor/Redirector)。预编译的二进制文件可以在压缩的归档文件 [https://github.com/matthewkastor/Redirector/archive/master.zip](https://github.com/matthewkastor/Redirector/archive/master.zip) 中找到，`Redirector-master\Gecko\Console Redirector\bin\Release`将所有 dll 和 exe 复制到任何你想要的地方。然后跑`Console Redirector.exe /?`
